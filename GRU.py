...
# Clean Feature Version
# Output format same as RNN
# Feature: raw + diff + rolling mean/std + diff lag
# Threshold: sweep on VAL, evaluate TEST
# =========================================================
...
# 預設 threshold，只是紀錄用
# 實際最後會使用 best_val_threshold
THRESHOLD = 0.30

# 建議先用你目前測起來比較穩的 [1.0, 1.0]
# 如果你要跟 RNN 的 [4.0, 1.0] 公平比較，可以改這裡
MANUAL_CLASS_WEIGHTS = [1.0, 1.0]

RANDOM_SEED = 42

MODEL_SAVE_PATH = "gru.pth"
RESULT_SAVE_PATH = "gru.xlsx"
...
def build_clean_features(df, sheet_name):
    ...
    roll_features = [c for c in feature_cols if "roll" in c]
    diff_lag_features = [
        c for c in feature_cols
        if "_diff" in c and c not in ["diff_in_out", "diff_mid_out", "diff_in_mid"]
    ]

    print("Rolling feature count:", len(roll_features))
    print("Diff lag feature count:", len(diff_lag_features))
    print("Slope feature count:", 0)
...
# =========================================================
# 13. Training with New Early Stopping
# =========================================================
...
# =========================================================
# 16. Threshold sweep on VAL and find best VAL F1
# =========================================================
...
# =========================================================
# 17. Evaluate TEST with best VAL threshold
# =========================================================
...
# =========================================================
# 18. 使用 best VAL threshold 產生 train / val / test 預測結果
# =========================================================
...
# =========================================================
# 20. 輸出 Excel：同一個 Sheet，依照 time 橫向整理 Group 1~4
# =========================================================
...
test_final_summary_df = pd.DataFrame([{
    ...
    "min_epochs": MIN_EPOCHS,
    "patience": PATIENCE,
    "manual_class_weights": str(MANUAL_CLASS_WEIGHTS),
    "model_type": "GRU",
