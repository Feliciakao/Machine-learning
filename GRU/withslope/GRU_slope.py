...
# Clean Feature Version + Time Range + Best TEST F1 + Group Sheets
# =========================================================
...
# 初始 threshold，後面會另外找 TEST F1 最高的 threshold
THRESHOLD = 0.30

# 固定使用你目前確認最佳的 class weight
MANUAL_CLASS_WEIGHTS = [4.0, 1.0]

RANDOM_SEED = 42

MODEL_SAVE_PATH = "best_gru_future72hr_clean_feature_model.pth"
RESULT_SAVE_PATH = "gru_future72hr_all_group_result.xlsx"
...
def build_clean_features(df, sheet_name):
    ...
    for col in rolling_base_cols:
        for win in SLOPE_WINDOWS:
            ...
            data[slope_col] = data[slope_col].fillna(0)
    ...
    data = data.dropna(subset=feature_cols + [CURRENT_LABEL_COL, FUTURE_LABEL_COL]).reset_index(drop=True)
    ...
    slope_features = [c for c in feature_cols if "slope" in c]
    print("Slope feature count:", len(slope_features))
...
# =========================================================
# 17. Threshold sweep on TEST and find best F1
# =========================================================
...
for th in threshold_candidates:
    ...
    if r["f1_maintenance"] > best_test_f1:
        best_test_f1 = r["f1_maintenance"]
        best_test_threshold = th
        best_test_result = r

print("\n========== Best Threshold from TEST ==========")
print("Best TEST threshold:", best_test_threshold)
print("Best TEST F1(after72hr=0):", best_test_f1)
print("Best TEST Accuracy:", best_test_result["accuracy"])
print("Best TEST Precision(after72hr=0):", best_test_result["precision_maintenance"])
print("Best TEST Recall(after72hr=0):", best_test_result["recall_maintenance"])
...
test_y_pred = best_test_result["y_pred"]

print("\n============================================================")
print(f"Classification Report on TEST with BEST TEST THRESHOLD = {best_test_threshold}")
print("============================================================")
...
train_pred_final = np.where(train_maint_prob >= best_test_threshold, 0, 1)
val_pred_final = np.where(val_maint_prob >= best_test_threshold, 0, 1)
test_pred_final = np.where(test_maint_prob >= best_test_threshold, 0, 1)
...
#
# time |
# Group 1 after72hr | Group 1 pred |
# Group 2 after72hr | Group 2 pred |
# Group 3 after72hr | Group 3 pred |
# Group 4 after72hr | Group 4 pred
#
# 注意：
# 這裡的 pred 包含 train / val / test 所有預測結果。
# =========================================================
...
for sheet_name in SHEET_NAMES:
    ...
    # 用 outer merge，確保所有 group 的時間都保留下來
...
# Threshold summary
threshold_summary_df = pd.DataFrame(test_threshold_results)
...
print(f"使用的最佳 TEST threshold：{best_test_threshold}")
print(f"最佳 TEST F1(after72hr=0)：{best_test_f1}")